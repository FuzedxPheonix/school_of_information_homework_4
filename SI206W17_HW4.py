...
file_read = open('nytimes_data.html',"r")

soup = BeautifulSoup(file_read, 'html.parser')

nytimes_headlines = []

new_list = soup.find_all(class_="story-heading",limit=10)

# print(new_list)

for heading in new_list:
	nytimes_headlines.append(heading.get_text())
...
# We worked on solving how to create an alternative way of finding and parasing the data in diffeent methods
...
people = soup.find_all(property="dc:title")
title_positions = soup.find_all("div", {"class":"field-name-field-person-titles"})
umsi_titles = {}

people_list = []
title_positions_list = []
for peeps in people:
	people_list.append(peeps.get_text())

for ok in title_positions:
	title_positions_list.append(ok.get_text())
